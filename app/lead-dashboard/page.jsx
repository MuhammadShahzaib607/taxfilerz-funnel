...
import {
  ...
  Inbox, ChevronDown, XCircle, PhoneIncoming
} from 'lucide-react';
...
export default function TaxFilerzDashboard() {
  ...
  const [contactLeads, setContactLeads] = useState([]); // Naya state ContactInfo ke liye
  ...
  const originalContacts = [...contactLeads];
  setContactLeads(contactLeads.filter(contact => contact._id !== id)); // Optimistic Update
  ...
  try {
    ...
    setContactLeads(originalContacts); // Error aaye toh wapas le aao
    alert("Deletion failed");
  }
};
  ...
  const filteredLeads = leads.filter((lead) => {
    ...
    if (activeTab === 'Email Start') return lead.sendMessages === true && matchSearch;
    if (activeTab === 'Email Stop') return lead.sendMessages === false && matchSearch;
    return matchSearch;
  });

  // Filter for Contact Info tab
    ...
    <>
      ...
      <div className="min-h-screen bg-[#F8FAFC] flex font-sans overflow-hidden text-[#1D2F52]">
        ...
        <aside className={`${isSidebarOpen ? 'w-72' : 'w-24'} bg-[#1D2F52] text-white transition-all duration-500 ease-in-out p-6 flex flex-col shadow-2xl m-4 rounded-[2.5rem] hidden md:flex relative overflow-hidden`}>
          ...
          <nav className="space-y-2 flex-1 z-10 overflow-y-auto pr-2 custom-scrollbar">
            {[
              ...
              { name: 'ContactInfo', icon: PhoneIncoming }, // Naya Tab
            ].map((item) => (
              <button
                key={item.name}
        ...
        <main className="flex-1 h-screen overflow-y-auto p-6 md:p-12 relative">
          ...
          {/* SEARCH BAR */}
          {(activeTab === 'Search' || activeTab === 'ContactInfo') && (
